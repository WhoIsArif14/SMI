        ...
        <div class="container mx-auto px-4 py-8">
            ...
            <div class="mb-6">
                <a href="{{ route('admin.experts.create') }}"
                    class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Tambah Expert Baru</a>
            </div>
                            ...
                            <tr>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    @if ($expert->photo_path)
                                        <img src="{{ asset('storage/' . $expert->photo_path) }}"
                                            alt="{{ $expert->name }}" class="w-16 h-16 object-cover rounded-full">
                                    @else
                                        <div
                                            class="w-16 h-16 bg-gray-200 rounded-full flex items-center justify-center text-gray-500">
                                            No Photo</div>
                                    @endif
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap">
                                        {{ Str::limit($expert->experience, 50) }}</p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-right">
                                    <div class="flex justify-end space-x-2">
                                        ...
                                        <form action="{{ route('admin.experts.destroy', $expert->id) }}" method="POST"
                                            onsubmit="return confirm('Apakah Anda yakin ingin menghapus expert ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
            ...
            <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4"
                ...
                <span class="absolute top-0 bottom-0 right-0 px-4 py-3">
                    <svg class="fill-current h-6 w-6 text-green-500" role="button" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <title>Close</title>
                        <path
                            d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l3.029-2.651-3.029-2.651a1.2 1.2 0 1 1 1.697-1.697l2.651 3.029 2.651-3.029a1.2 1.2 0 0 1 1.697 1.697l-3.029 2.651 3.029 2.651a1.2 1.2 0 0 1 0 1.697z" />
                    </svg>
                </span>
            </div>
        @endif
            ...
            <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert">
                ...
                <span class="absolute top-0 bottom-0 right-0 px-4 py-3">
                    <svg class="fill-current h-6 w-6 text-red-500" role="button" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <title>Close</title>
                        <path
                            d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l3.029-2.651-3.029-2.651a1.2 1.2 0 1 1 1.697-1.697l2.651 3.029 2.651-3.029a1.2 1.2 0 0 1 0 1.697z" />
                    </svg>
                </span>
            </div>
        @endif
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
            <div class="flex justify-end mb-6">
                <a href="{{ route('admin.experts.create') }}"
                    ...
                    <svg class="me-2 -ms-0.5 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            ...
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                    @if ($expert->photo_path)
                                        <img src="{{ asset('storage/' . $expert->photo_path) }}"
                                            alt="{{ $expert->name }}"
                                            class="w-16 h-16 object-cover rounded-full shadow-lg ring-2 ring-teal-200">
                                    @else
                                        <div
                                            class="w-16 h-16 bg-gradient-to-br from-teal-400 to-teal-600 rounded-full flex items-center justify-center text-white font-bold shadow-lg">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                    {{ $expert->university ?? '-' }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-300">
                                    <p class="max-w-xs truncate">{{ Str::limit($expert->experience, 50) }}</p>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('admin.experts.edit', $expert->id) }}"
                                        class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-600 me-3">Edit</a>
                                    <form action="{{ route('admin.experts.destroy', $expert->id) }}" method="POST"
                                        class="inline delete-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
                                            class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-600 delete-btn"
                                            data-name="{{ $expert->name }}">Hapus</button>
                                    </form>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6"
                                    ...
                                    Belum ada expert yang ditambahkan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="mt-3 text-center">
            ...
            <div class="items-center px-4 py-3">
                <button id="confirmDelete"
                    class="px-4 py-2 bg-red-500 text-white text-base font-medium rounded-md w-24 mr-2 hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-300 transition duration-150 ease-in-out">
                    Hapus
                </button>
                <button id="cancelDelete"
                    class="px-4 py-2 bg-gray-500 text-white text-base font-medium rounded-md w-24 hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-300 transition duration-150 ease-in-out">
                    Batal
                </button>
            </div>
        </div>
